...
import CrtPowerTransition from './components/CrtPowerTransition';
import { playWin95Startup, playModemDialup, playCrtPowerSound } from './lib/audioSynth';
...
export default function App() {
  ...
  const [isCrtTransitioning, setIsCrtTransitioning] = useState(false);
  const [crtTransitionKey, setCrtTransitionKey] = useState(0);

  const handleThemeChange = (newTheme: ThemeMode) => {
    if (newTheme === themeMode) return;
    setThemeMode(newTheme);
    setIsCrtTransitioning(true);
    setCrtTransitionKey(prev => prev + 1);
    playCrtPowerSound();
    setTimeout(() => {
      setIsCrtTransitioning(false);
    }, 650);
  };

  const [readingFilter, setReadingFilter] = useState<'none' | 'sepia' | 'dim'>('none');
  const [soundMode, setSoundMode] = useState<'muted' | 'modem' | 'win95'>(
    () => (safeStorage.getItem('web_history_retro_sound') as any) || 'muted'
  );
  ...
  const handleSoundModeChange = (mode: 'muted' | 'modem' | 'win95') => {
    setSoundMode(mode);
    safeStorage.setItem('web_history_retro_sound', mode);
    if (mode === 'win95') {
      playWin95Startup();
    } else if (mode === 'modem') {
      playModemDialup();
  ...
  return (
    <>
      <CrtPowerTransition key={crtTransitionKey} isActive={isCrtTransitioning} theme={themeMode} />
      <div
      ...
      <div className={style.containerClass} id="app-container">
        ...
        <div className="flex flex-col min-h-[580px] flex-1">
          ...
          {/* Navigation bar with direct select dropdown and inline expanding categories */}
            ...
            <div className="space-y-3">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                <span className="text-[10px] font-mono font-extrabold uppercase tracking-widest text-slate-400 block">
                  📂 NAvigation MUSÉE INTERACTIF ({navItems.length} Sections) :
                </span>
                {/* Direct Select Box for quick unambiguous navigation */}
                <select
                  value={activeTab}
                  onChange={(e) => setActiveTab(e.target.value as AppTab)}
                  className="px-3 py-1.5 bg-slate-900 text-slate-200 border border-slate-700 rounded-lg font-mono text-xs font-bold cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  id="nav-quick-selector"
                >
                  {navGroups.map((group) => (
                    <optgroup key={group.label} label={group.label} className="bg-slate-900 text-slate-300 font-bold">
                      {group.items.map((itemId) => {
                        const item = navItems.find((i) => i.id === itemId);
                        if (!item) return null;
                        return (
                          <option key={item.id} value={item.id} className="bg-slate-900 text-slate-100 font-normal py-1">
              ...
              <nav className="flex items-center gap-2 flex-wrap pb-1">
                {navGroups.map((group) => {
                  ...
                  return (
                    <div key={group.label} className="flex flex-col gap-1">
                      <button
                        ...
                        className={`px-3 py-1.5 border rounded-lg flex items-center gap-2 transition-all cursor-pointer text-left ${
                          isGroupActive
                            ? 'bg-indigo-600 text-white border-indigo-500 font-bold shadow-md'
                            : 'bg-slate-900/80 text-slate-300 border-slate-750 hover:bg-slate-800'
                        }`}
                      >
                        <Icon className="w-3.5 h-3.5 shrink-0" />
                        <span className="text-xs tracking-tight">{group.label}</span>
                        {isOpen ? <ChevronUp className="w-3 h-3 ml-1" /> : <ChevronDown className="w-3 h-3 ml-1" />}
                      </button>

                      {isOpen && (
                        <div className="flex flex-wrap gap-1 p-2 bg-slate-950/90 border border-slate-800 rounded-lg mt-1 w-full max-w-full">
                          {group.items.map((itemId) => {
                            const item = navItems.find((i) => i.id === itemId);
                            if (!item) return null;
                            ...
                            return (
                              <button
                                ...
                                className={`px-2.5 py-1 text-xs rounded-md transition-colors font-medium text-left ${
                                  isSelected
